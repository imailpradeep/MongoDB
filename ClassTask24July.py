...
XXX    5. Store this dataset into mongodb
XXX    6. in sql task try to perform left join operation with attrubute dataset and dress dataset on column Dress_ID
...
cursor.execute('SELECT * FROM attribute;') # ('SELECT * FROM dresssales;')
...
cursor.execute('SELECT * FROM attribute WHERE Recommendation = 0;')
record = cursor.fetchall()
for i in record:
    print(i)
